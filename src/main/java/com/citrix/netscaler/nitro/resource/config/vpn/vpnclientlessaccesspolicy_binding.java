...
import com.citrix.netscaler.nitro.resource.config.vpn.vpnclientlessaccesspolicy_vpnvserver_binding;
import com.citrix.netscaler.nitro.resource.config.vpn.vpnclientlessaccesspolicy_vpnglobal_binding;
...
public class vpnclientlessaccesspolicy_binding extends base_resource
{
	...
	*/
	protected base_resource[] get_nitro_response(nitro_service service, String response) throws Exception{
		vpnclientlessaccesspolicy_binding_response result = (vpnclientlessaccesspolicy_binding_response) service.get_payload_formatter().string_to_resource(vpnclientlessaccesspolicy_binding_response.class, response);
	...
	*/
	...
	/**
	* Use this API to fetch vpnclientlessaccesspolicy_binding resources of given names .
	*/
	public static vpnclientlessaccesspolicy_binding[] get(nitro_service service, String name[]) throws Exception{
		if (name !=null && name.length>0) {
			vpnclientlessaccesspolicy_binding response[] = new vpnclientlessaccesspolicy_binding[name.length];
			vpnclientlessaccesspolicy_binding obj[] = new vpnclientlessaccesspolicy_binding[name.length];
			for (int i=0;i<name.length;i++) {
				obj[i] = new vpnclientlessaccesspolicy_binding();
				obj[i].set_name(name[i]);
				response[i] = (vpnclientlessaccesspolicy_binding) obj[i].get_resource(service);
			}
			return response;
		}
		return null;
