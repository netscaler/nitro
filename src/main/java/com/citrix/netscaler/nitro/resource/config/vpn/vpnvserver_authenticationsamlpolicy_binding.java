...
* The expression evaluates to a priority number that is between the current policy's priority number (say, 30) and the highest priority number (say, 100), but does not match any configured priority number (for example, the expression evaluates to the number 85). This example assumes that the priority number increments by 10 for every successive policy, and therefore a priority number of 85 does not exist in the policy label.
	...
	*/
	...
	public static base_response add(nitro_service client, vpnvserver_authenticationsamlpolicy_binding resource) throws Exception {
		vpnvserver_authenticationsamlpolicy_binding updateresource = new vpnvserver_authenticationsamlpolicy_binding();
	...
	public static base_responses add(nitro_service client, vpnvserver_authenticationsamlpolicy_binding resources[]) throws Exception {
		base_responses result = null;
		if (resources != null && resources.length > 0) {
			vpnvserver_authenticationsamlpolicy_binding updateresources[] = new vpnvserver_authenticationsamlpolicy_binding[resources.length];
			for (int i=0;i<resources.length;i++){
				updateresources[i] = new vpnvserver_authenticationsamlpolicy_binding();
	...
	public static base_response delete(nitro_service client, vpnvserver_authenticationsamlpolicy_binding resource) throws Exception {
		vpnvserver_authenticationsamlpolicy_binding deleteresource = new vpnvserver_authenticationsamlpolicy_binding();
	...
	public static base_responses delete(nitro_service client, vpnvserver_authenticationsamlpolicy_binding resources[]) throws Exception {
		base_responses result = null;
		if (resources != null && resources.length > 0) {
			vpnvserver_authenticationsamlpolicy_binding deleteresources[] = new vpnvserver_authenticationsamlpolicy_binding[resources.length];
			for (int i=0;i<resources.length;i++){
				deleteresources[i] = new vpnvserver_authenticationsamlpolicy_binding();
